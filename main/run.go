...
func main() {
	...
	if err != nil {
		...
		fmt.Println("Created Discord Session successfully")
	}
	k, err := ken.New(dg, ken.Options{
		CommandStore: store.NewDefault(),
	})
	must(err)
	must(k.RegisterCommands(new(pHelpCommand)))
	must(k.RegisterCommands(new(HelpCommand)))
	must(k.RegisterCommands(new(Generate)))
	must(k.RegisterCommands(new(Bob)))
	must(k.RegisterCommands(new(EvilBob)))
	must(k.RegisterCommands(new(Question)))
	must(k.RegisterCommands(new(wipebob)))
	must(k.RegisterCommands(new(wipeEvilBob)))
	must(k.RegisterCommands(new(Summarize)))
	//defer k.Unregister()
	err = dg.Open()
...
//		temp := 0.85
//		length := 30
//		generate(length, temp, top_p, str2, false)
//		return
//	}
//
...
//		temp := 1.1
//		length := 70
//		generatePersonality(length, temp, top_p, str2,"Evil Bob")
//		return
//	}
//
...
//		temp := 0.4
//		length := 75
//		generatePersonality(length, temp, top_p, str2, "Bob")
//		return
//	}
...
//		temp := 0.35
//		length := 85
//		generate(length, temp, top_p, str2, true)
//		return
//	}
